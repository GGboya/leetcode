...
def update(qqlist, cursor, dic, mode, nostudy):
    for ip, qq, score, cur_ac_cnt, name, title in qqlist:
        ...
        if row: # 数据库有对应记录，那么就去更新对应的
            ...
            if mode == 1:
                ...
                cursor.execute("UPDATE users SET status = %s, ac_count = %s, homepage=%s where qq = %s", (status, cur_ac_cnt,ip,qq))
                continue
            ...
            if mode == 2:
                ...
                if isStrongUser:
                    cursor.execute(
                        ...
                        (score, status, ip, name, cur_ac_cnt, ac_count, qq))
                else:
                    cursor.execute("UPDATE users SET level = %s, title = %s, score = %s, status = %s, homepage = %s, username = %s, ac_count = %s, yesterday_count = %s   WHERE qq = %s",
                                   (level, title, score, status, ip, name, cur_ac_cnt, ac_count, qq))
                continue
            ...
            if mode == 3:
                ...
                if isStrongUser:
                    cursor.execute(
                        ...
                        (score, status, ip, name, cur_ac_cnt, ac_count, cur_ac_cnt, qq))
                else:
                    cursor.execute(
                        ...
                        (level, title, score, status, ip, name, cur_ac_cnt, ac_count, cur_ac_cnt, qq))
        else:
            # 如果QQ号不存在，则插入一条新记录
